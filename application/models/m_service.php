...
class m_service extends CI_Model
{
	...
	public function MTampilPembayaran($where)
	{
		...
		return $this->db
			...
			->from('tb_transaksi')
			->where($where)
			->join('tb_member', 'tb_transaksi.id_member = tb_member.id_member', 'left')
			->get()
			->result_array();
	}
	...
	public function MProsesBayar($id_transaksi, $total, $total_harga)
	{
		$this->db->where('id_transaksi', $id_transaksi);

		if ($total >= $total_harga) {
			return $hasil = $this->db->update('tb_transaksi', [
				'bayar_transaksi' => $total,
	...
	public function MHapusKeranjang($id_detail_transaksi)
	{
		...
		return $this->db->delete('tb_detail_transaksi');
	}
