            ...
            <div class="row mt-3">
                <div class="col-md-12">
                    <table id="tabelpembayaran" class="table table-striped table-bordered" style="width:100%">
                        <thead align="center" class="bg-info text-white">
                            <tr>
                                <th>No</th>
                                <th>Nama Member</th>
                                <th>Kode Invoice</th>
                                ...
                                <tr>
                                    ...
                                    <td align="center">
                                        <?php if ($d['status_pembayaran'] != 'dibayar') : ?>
                                            <a href="<?php echo site_url('C_service/CProsesTampilPembayaran/' . $d['id_transaksi']) ?>" class="btn btn-outline-success"> Bayar </a>
                                            <a href="<?php echo site_url('C_service/CHapusPembayaran/' . $d['id_transaksi']) ?>" class="btn btn-outline-success" onclick="return confirm('Yakin ingin menghapus')"> Hapus </a>
                                        <?php endif ?>
                                        <?php if ($d['status_transaksi'] == 'proses') : ?>
                                            <a href="<?php echo site_url('C_service/CTampilSelesai/' . $d['id_transaksi']) ?>" class="btn btn-outline-success" onclick="return confirm('Yakin Apakah Barang Telah selesai Dicuci')"> Selesai </a>
                                        <?php endif ?>
                                        <?php if (($d['status_pembayaran'] == 'dibayar') && ($d['status_transaksi'] != 'diambil') && ($d['status_transaksi'] != 'proses')) : ?>
                                            <a href="<?php echo site_url('C_service/CProsesTampilPengambilan/' . $d['id_transaksi']) ?>" class="btn btn-outline-success"> Ambil </a>
                                        <?php endif ?>
                                    </td>
                                </tr>
                            <?php endforeach ?>
